import numpy as np
from ex2_trainModel import theta, X_test, y_test
from ex2_costfunction import costFunction as cf
from ex2_sigmoidFunction import sigmoidFunction as sf
import pandas as pd
import ex2_crossValTest

# Predict y_pred_test
y_pred_test = sf.sigmoidFunction(X_test, theta)
y_pred_test = np.where(y_pred_test>=0.5,1,0)

# Check Precision, Recall, F1 Score, and Accuracy
df_pred = pd.DataFrame(y_pred_test, columns=['predicted_val'])
df_act = pd.DataFrame(y_test, columns=['actual_val'])
...
print('F1 Score based on test data: {}'.format(f1score))

# Print cost function based on cross validation data
cost = cf.costFunction(X_test, theta, y_test)
print('Cost function based test data: {}'.format(cost))